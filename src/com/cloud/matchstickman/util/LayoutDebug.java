...
public class LayoutDebug implements InputProcessor{
	...
	public boolean keyDown(int keycode) {
		...
		case Input.Keys.NUM_7:
			if(actors[debugIndex] instanceof Label){
				((Label)actors[debugIndex]).setFontScale(((Label)actors[debugIndex]).getFontScaleX()-0.01f);
			}
			break;
	...
	private void outPut(){
		...
		if(actors[debugIndex] instanceof Label){
			Label label=(Label)actors[debugIndex];
			System.out.println("FontScale="+label.getFontScaleX());
		}
	}
	...
	class DebugActor extends Actor{
		...
		public void addFont(){
			...
			infoLabel.setPosition(0,infoLabel.getPrefWidth()/2);
		}
		@Override
		public void act(float delta) {
			...
			if(infoLabel!=null){
				String s=" ";
				if(actors[debugIndex] instanceof Label){
					s="sca="+((Label)actors[debugIndex]).getFontScaleX();
				}
				infoLabel.setText("X="+(int)getX()+" Y="+(int)getY()+s);
			}
		}
		@Override
